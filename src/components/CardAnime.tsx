...
function CardAnime(props: CardProps) {
  const { id, title, episode, imageSrc, genres = [], isFullData } = props
  const limitedGenres = genres.slice(0, 3)
  return (
    <Link to={`/anime/${id}`} className="group w-full shadow-neutral-50 relative delay-0 duration-0 overflow-hidden border border-neutre rounded-tl-3xl rounded-br-3xl max-h-[550px]">
      <img
